...
def validate_container_file(path: Path) -> None:
    ...
    required_fragments = {
        ...
        "fun searchByTitleSearch(query: String): List<TestIndexesPost>": "post title token search",
        "addGroup(searchTitleSearch, token, row)": "post title search postings",
        ...
        "fun getUserLookupUser(row: TestIndexesUserLookup): TestIndexesUser?": "table ref unique helper",
        "fun findUserLookupDisplayMatches(row: TestIndexesUserLookup): List<TestIndexesUser>": "table ref group helper",
        "fun findPostSearchTitleMatches(row: TestIndexesPostSearch): List<TestIndexesPost>": "table ref search helper",
        "return users.getById(key)": "user navigation lookup",
        "return tags.getById(key)": "tag navigation lookup",
        "users.getByDisplayNameId(listOf(row.display_name, row.user_id))": "table ref composite lookup",
...
def validate_binary_refs_file(path: Path) -> None:
    ...
    if "07_indexes" in path.parts:
        required_fragments = {
            ...
            "fun searchByTitleSearch(query: String): List<BinaryRef<TestIndexesPost>>": "post title token search",
