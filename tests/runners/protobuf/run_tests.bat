...
REM PolyGen Protobuf Integration Test Runner for Windows
...
echo === PolyGen Protobuf Integration Tests ===
echo.
...
for %%T in (%TEST_CASES%) do (
    ...
    if not exist "!SCHEMA_PATH!" (
        ...
        set VALIDATION_LOG=!TEST_OUTPUT!\protobuf_validation.log
        echo   Generating Protobuf schema...
        "%POLYGEN%" --schema-path "!SCHEMA_PATH!" --lang protobuf --output-dir "!TEST_OUTPUT!" --templates-dir "%PROJECT_ROOT%\templates" > "!GENERATION_LOG!" 2>&1
        if errorlevel 1 (
            echo   FAILED ^(generation error^)
            type "!GENERATION_LOG!"
            set /a FAILED+=1
        ) else if not exist "!TEST_OUTPUT!\protobuf" (
            echo   FAILED ^(no protobuf output directory^)
            set /a FAILED+=1
        ) else if not exist "!TEST_OUTPUT!\protobuf\*.proto" (
            echo   FAILED ^(no protobuf files generated^)
            set /a FAILED+=1
        ) else (
            echo   Validating proto3 structure...
            set CASE_FAILED=0
            if exist "!VALIDATION_LOG!" del /q "!VALIDATION_LOG!" >nul 2>&1
            for %%F in ("!TEST_OUTPUT!\protobuf\*.proto") do (
                python "%SCRIPT_DIR%validate_proto.py" "%%~fF" >> "!VALIDATION_LOG!" 2>&1
                if errorlevel 1 set CASE_FAILED=1
            )

            if !CASE_FAILED! neq 0 (
                echo   FAILED ^(proto validation error^)
                type "!VALIDATION_LOG!"
                set /a FAILED+=1
            ) else (
                echo   PASSED
