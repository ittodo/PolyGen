...
for case_name in "${TEST_CASES[@]}"; do
    ...
    if ! "$POLYGEN" --schema-path "$schema_path" --lang mysql --output-dir "$test_output" --templates-dir "$PROJECT_ROOT/templates" >"$generation_log" 2>&1; then
    ...
    ddl_path="$test_output/mysql/schema.sql"
    validation_log="$test_output/mysql_validation.log"
    if [[ ! -f "$ddl_path" ]]; then
        echo "  FAILED (MySQL DDL file not found)"
        failed=$((failed + 1))
    elif "$PYTHON_BIN" "$SCRIPT_DIR/validate_mysql.py" "$ddl_path" >"$validation_log" 2>&1; then
        echo "  PASSED"
        passed=$((passed + 1))
    else
