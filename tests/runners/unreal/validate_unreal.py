...
def validate_registry(path: Path) -> None:
    ...
    if "07_indexes" not in path.parts:
        ...
        required_fragments = {
            ...
            "void FindUserLookupDisplayMatches(const FPolygenUserLookup& Row, TArray<FPolygenUser>& OutRows) const": "table ref group navigation API",
            "GetUsersByDisplayName(Row.display_query, OutRows);": "table ref group navigation lookup",
            "void FindPostSearchTitleMatches(const FPolygenPostSearch& Row, TArray<FPolygenPost>& OutRows) const": "table ref search navigation API",
            "OutRows = SearchPostByTitleSearch(Row.query);": "table ref search navigation lookup",
