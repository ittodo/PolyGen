...
REM PolyGen PostgreSQL DDL Integration Test Runner for Windows
...
echo === PolyGen PostgreSQL DDL Integration Tests ===
echo.
...
set VALIDATION_LOG=!TEST_OUTPUT!\postgresql_validation.log

echo   Generating PostgreSQL DDL...
"!POLYGEN!" --schema-path "!SCHEMA_PATH!" --lang postgresql --output-dir "!TEST_OUTPUT!" --templates-dir "!PROJECT_ROOT!\templates" > "!GENERATION_LOG!" 2>&1
if errorlevel 1 (
...
if not exist "!TEST_OUTPUT!\postgresql\schema.sql" (
    echo   FAILED ^(PostgreSQL DDL file not found^)
    set /a FAILED+=1
    exit /b 0
)

echo   Validating PostgreSQL DDL...
python "!SCRIPT_DIR!validate_postgresql.py" "!TEST_OUTPUT!\postgresql\schema.sql" > "!VALIDATION_LOG!" 2>&1
if errorlevel 1 (
    echo   FAILED ^(PostgreSQL DDL validation error^)
    type "!VALIDATION_LOG!"
    set /a FAILED+=1
) else (
    echo   PASSED
