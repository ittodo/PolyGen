...
def validate_container_file(path: Path) -> None:
    ...
    required_fragments = {
        ...
        "func searchByTitleSearch(_ query: String) -> [TestIndexesPost]": "post title token search",
        "addGroup(&searchTitleSearch, key: AnyHashable(token), row: row)": "post title search postings",
        ...
        "func getUserLookupUser(_ row: TestIndexesUserLookup) -> TestIndexesUser?": "table ref unique helper",
        "func findUserLookupDisplayMatches(_ row: TestIndexesUserLookup) -> [TestIndexesUser]": "table ref group helper",
        "func findPostSearchTitleMatches(_ row: TestIndexesPostSearch) -> [TestIndexesPost]": "table ref search helper",
        "return users.getById(key)": "user navigation lookup",
        "return tags.getById(key)": "tag navigation lookup",
        "users.getByDisplayNameId([AnyHashable(row.display_name), AnyHashable(row.user_id)])": "table ref composite lookup",
        "users.findByDisplayName(row.display_query)": "table ref group lookup",
        "posts.searchByTitleSearch(row.query)": "table ref search lookup",
...
def validate_binary_refs_file(path: Path) -> None:
    ...
    if "07_indexes" in path.parts:
        required_fragments = {
            ...
            "func searchByTitleSearch(_ query: String) throws -> [BinaryRef<TestIndexesPost>]": "post title token search",
