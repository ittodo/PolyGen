...
REM PolyGen Python Integration Test Runner for Windows
...
for %%T in (%TEST_CASES%) do (
    ...
    if not exist "!SCHEMA_PATH!" (
        ...
        set VALIDATION_LOG=!TEST_OUTPUT!\python_validation.log
        echo   Generating Python code...
        "%POLYGEN%" --schema-path "!SCHEMA_PATH!" --lang python --output-dir "!TEST_OUTPUT!" --templates-dir "%PROJECT_ROOT%\templates" > "!GENERATION_LOG!" 2>&1
        if errorlevel 1 (
            echo   FAILED ^(generation error^)
            type "!GENERATION_LOG!"
            set /a FAILED+=1
        ) else if not exist "!TEST_OUTPUT!\python" (
            echo   FAILED ^(no python output directory^)
            set /a FAILED+=1
        ) else if not exist "!TEST_OUTPUT!\python\*.py" (
            echo   FAILED ^(no python files generated^)
            set /a FAILED+=1
        ) else (
            echo   Compiling generated Python...
            set CASE_FAILED=0
            if exist "!VALIDATION_LOG!" del /q "!VALIDATION_LOG!" >nul 2>&1
            for %%F in ("!TEST_OUTPUT!\python\*.py") do (
                python -m py_compile "%%~fF" >> "!VALIDATION_LOG!" 2>&1
                if errorlevel 1 set CASE_FAILED=1
            )

            if !CASE_FAILED! neq 0 (
                echo   FAILED ^(py_compile error^)
                type "!VALIDATION_LOG!"
                set /a FAILED+=1
            ) else (
