...
REM PolyGen MessagePack Integration Test Runner for Windows
...
for %%T in (%TEST_CASES%) do (
    ...
    if not exist "!SCHEMA_PATH!" (
        ...
        set VALIDATION_LOG=!TEST_OUTPUT!\messagepack_validation.log
        echo   Generating MessagePack descriptor...
        "%POLYGEN%" --schema-path "!SCHEMA_PATH!" --lang messagepack --output-dir "!TEST_OUTPUT!" --templates-dir "%PROJECT_ROOT%\templates" > "!GENERATION_LOG!" 2>&1
        if errorlevel 1 (
            echo   FAILED ^(generation error^)
            type "!GENERATION_LOG!"
            set /a FAILED+=1
        ) else if not exist "!TEST_OUTPUT!\messagepack" (
            echo   FAILED ^(no messagepack output directory^)
            set /a FAILED+=1
        ) else if not exist "!TEST_OUTPUT!\messagepack\*.messagepack.json" (
            echo   FAILED ^(no messagepack descriptors generated^)
            set /a FAILED+=1
        ) else (
            echo   Validating descriptor JSON...
            set CASE_FAILED=0
            if exist "!VALIDATION_LOG!" del /q "!VALIDATION_LOG!" >nul 2>&1
            for %%F in ("!TEST_OUTPUT!\messagepack\*.messagepack.json") do (
                python "%SCRIPT_DIR%validate_descriptor.py" "%%~fF" >> "!VALIDATION_LOG!" 2>&1
                if errorlevel 1 set CASE_FAILED=1
            )

            if !CASE_FAILED! neq 0 (
                echo   FAILED ^(descriptor validation error^)
                type "!VALIDATION_LOG!"
                set /a FAILED+=1
            ) else (
                echo   PASSED
