...
```
examples/
├── quickstart.poly         # 바로 실행 가능한 최소 예제
├── quickstart.sources.toml # quickstart CSV load source 설정
├── data/items.csv          # quickstart 샘플 CSV 데이터
├── run-quickstart.ps1      # runtime binary 기반 quickstart 실행 스크립트
├── run-feature-example.ps1 # runtime binary 기반 기능별 예제 실행 스크립트
├── features/               # 기능별 최소 예제 모음
│   ├── constraints.poly
│   ├── relations_indexes.poly
│   ├── sources.poly
...
### quickstart.poly
- **용도**: 사용자가 가장 먼저 실행할 최소 예제
- **내용**:
  - `demo.catalog.Item` 테이블
  - `Rarity` enum
  - `@datasource("sqlite")`, `@index`, 기본 제약조건
  - `quickstart.sources.toml`을 통한 CSV load path 분리
- **실행 방법**:
  ```powershell
...
- **소스 개발 중 실행 방법**:
  ```bash
  cargo run -- --schema-path examples/quickstart.poly --sources examples/quickstart.sources.toml --lang csharp
  ```

### features/
- **용도**: 기능별로 하나씩 확인할 수 있는 작은 예제 모음
- **내용**:
  - `constraints.poly`: field constraints
  - `relations_indexes.poly`: foreign key, reverse relation alias, `@index`
  - `sources.poly` + `sources.sources.toml`: load source sidecar
  - `pack_embed.poly`: `@pack` embed serialization
  - `search.poly`: field-level `@search`
- **사용자 문서**: `docs/examples/features.md`
  - 상세 문서: `docs/examples/constraints.md`, `docs/examples/relations-indexes.md`, `docs/examples/sources.md`, `docs/examples/pack-embed.md`, `docs/examples/search.md`
- **실행 예시**:
  ```powershell
  .\examples\run-feature-example.ps1 -Feature constraints
  .\examples\run-feature-example.ps1 -Feature all
  ```
