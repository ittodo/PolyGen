...
/// Policy for handling destructive migration changes.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum DestructiveChangePolicy {
    /// Keep destructive SQL commented and emit warnings.
    Warn,
    /// Fail before writing migration SQL if destructive changes are present.
    Fail,
    /// Emit executable destructive SQL where PolyGen can express it directly.
    Allow,
}

impl DestructiveChangePolicy {
    ...
    pub fn parse(value: &str) -> anyhow::Result<Self> {
        match value.to_ascii_lowercase().as_str() {
            ...
            "allow" => Ok(Self::Allow),
            _ => anyhow::bail!(
                "지원하지 않는 파괴적 변경 정책: {}. warn, fail, allow 중 하나를 사용하세요.",
                value
            ),
        }
    }
...
impl MigrationDiff {
    ...
    pub fn to_sqlite_sql(&self) -> String {
        self.to_sqlite_sql_with_policy(DestructiveChangePolicy::Warn)
    }

    /// Generate SQLite migration SQL from the diff using a destructive change policy.
    pub fn to_sqlite_sql_with_policy(&self, destructive_policy: DestructiveChangePolicy) -> String {
        let mut sql = String::new();
        sql.push_str("-- Generated by PolyGen - Migration Diff\n");
        sql.push_str("-- Compare baseline schema with current schema\n\n");
        sql.push_str(polygen_schema_table_sql());
        sql.push_str(polygen_migrations_table_sql());
        sql.push('\n');

        self.append_sqlite_changes(&mut sql, destructive_policy);
        sql
    }

    /// Generate SQLite migration SQL and record the target schema metadata.
    pub fn to_sqlite_sql_with_schema(&self, schema: &SchemaContext) -> String {
        self.to_sqlite_sql_with_schema_and_policy(schema, DestructiveChangePolicy::Warn)
    }

    /// Generate SQLite migration SQL, metadata, and destructive policy handling.
    pub fn to_sqlite_sql_with_schema_and_policy(
        &self,
        schema: &SchemaContext,
        destructive_policy: DestructiveChangePolicy,
    ) -> String {
        let mut sql = String::new();
        sql.push_str("-- Generated by PolyGen - Migration Diff\n");
        sql.push_str("-- Compare baseline schema with current schema\n\n");
        sql.push_str(polygen_schema_table_sql());
        sql.push_str(polygen_migrations_table_sql());
        sql.push_str(&polygen_schema_upsert_sql(schema));
        sql.push_str(&polygen_migration_insert_sql(schema, "migration"));
        sql.push('\n');
    ...
    pub fn to_mysql_sql(&self) -> String {
        self.to_mysql_sql_with_policy(DestructiveChangePolicy::Warn)
    }

    /// Generate MySQL migration SQL from the diff using a destructive change policy.
    pub fn to_mysql_sql_with_policy(&self, destructive_policy: DestructiveChangePolicy) -> String {
        let mut sql = String::new();
        ...
        for change in &self.changes {
            match change {
                ...
                SchemaChange::TableRemoved {
                    ...
                    if destructive_policy == DestructiveChangePolicy::Allow {
    ...
    /// Returns true if the diff contains destructive or data-risking operations.
    ...
    /// Human-readable destructive change descriptions.
    pub fn destructive_change_descriptions(&self) -> Vec<String> {
        self.changes
            .iter()
            .filter_map(|change| match change {
                SchemaChange::TableRemoved {
                    ...
                    "table removed: {}",
                    make_table_name(namespace, table_name)
                )),
                SchemaChange::ColumnRemoved {
                    table_name,
                    namespace,
...
impl MigrationDiff {
    fn append_sqlite_changes(&self, sql: &mut String, destructive_policy: DestructiveChangePolicy) {
        // Add warnings as comments
        if !self.warnings.is_empty() {
            sql.push_str("-- WARNINGS:\n");
            for warning in &self.warnings {
...
fn polygen_schema_table_sql() -> &'static str {
...
fn polygen_migrations_table_sql() -> &'static str {
...
fn polygen_schema_upsert_sql(schema: &SchemaContext) -> String {
    let metadata =
        build_schema_metadata(schema).unwrap_or_else(|_| crate::schema_metadata::SchemaMetadata {
            hash: String::new(),
            json: "{}".to_string(),
        });
    format!(
        ...
        metadata.hash,
        sql_string_literal(&metadata.json)
    )
}

fn polygen_migration_insert_sql(schema: &SchemaContext, description: &str) -> String {
    let metadata =
        build_schema_metadata(schema).unwrap_or_else(|_| crate::schema_metadata::SchemaMetadata {
            hash: String::new(),
            json: "{}".to_string(),
...
mod tests {
    ...
    #[test]
    fn test_sqlite_migration_creates_polygen_schema_table() {
        ...
        let metadata = build_schema_metadata(&schema).expect("metadata should serialize");
        let sql = diff.to_sqlite_sql_with_schema(&schema);

        assert!(sql.contains("CREATE TABLE IF NOT EXISTS _polygen_schema"));
        assert!(sql.contains("INSERT INTO _polygen_schema"));
        assert!(sql.contains("CREATE TABLE IF NOT EXISTS _polygen_migrations"));
        assert!(sql.contains("INSERT INTO _polygen_migrations"));
        assert!(sql.contains("schema_hash TEXT"));
        assert!(sql.contains("schema_json TEXT"));
        assert!(sql.contains(&format!("'{}'", metadata.hash)));
        assert!(sql.contains("-- No changes detected."));
    }
}
