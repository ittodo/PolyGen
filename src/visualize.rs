...
pub struct FieldVisualization {
    ...
    /// Foreign key target table FQN, if this field references another table.
    #[serde(rename = "foreignKeyTarget")]
            ...
            if let StructItem::Field(field) = item {
                ...
                fields.push(FieldVisualization {
                    ...
                    foreign_key_target: field
                        .foreign_key
                        .as_ref()
                        .map(|fk| fk.target_table_fqn.clone()),
                    constraints,
                });
            }
        }
...
/// Generate a Markdown schema reference from visualization data.
pub fn to_markdown(viz: &SchemaVisualization) -> String {
    let mut output = String::new();

    output.push_str("# Schema Reference\n\n");
    output.push_str("Generated by PolyGen.\n\n");

    output.push_str("## Summary\n\n");
    output.push_str("| Metric | Count |\n");
    output.push_str("|--------|------:|\n");
    output.push_str(&format!("| Tables | {} |\n", viz.stats.table_count));
    output.push_str(&format!("| Fields | {} |\n", viz.stats.field_count));
    output.push_str(&format!("| Relations | {} |\n", viz.stats.relation_count));
    output.push_str(&format!("| Namespaces | {} |\n", viz.stats.namespace_count));
    output.push('\n');

    if !viz.stats.tables_by_namespace.is_empty() {
        let mut namespaces: Vec<_> = viz.stats.tables_by_namespace.iter().collect();
        namespaces.sort_by(|a, b| a.0.cmp(b.0));

        output.push_str("## Namespaces\n\n");
        output.push_str("| Namespace | Tables |\n");
        output.push_str("|-----------|-------:|\n");
        for (namespace, count) in namespaces {
            ...
            output.push_str(&format!("| `{}` | {} |\n", escape_markdown(display), count));
        }
        output.push('\n');
    }

    if viz.stats.relation_count > 0 {
        output.push_str("## ER Diagram\n\n");
        output.push_str("```mermaid\n");
        output.push_str(&to_mermaid(viz));
        output.push_str("```\n\n");
    }

    output.push_str("## Tables\n\n");

    if viz.tables.is_empty() {
        output.push_str("No tables defined.\n");
        return output;
    }

    for table in &viz.tables {
        output.push_str(&format!("### `{}`\n\n", escape_markdown(&table.fqn)));
        output.push_str(&format!("- Name: `{}`\n", escape_markdown(&table.name)));
        output.push_str(&format!(
            "- Namespace: `{}`\n",
            escape_markdown(if table.namespace.is_empty() {
                "(root)"
            } else {
                &table.namespace
            })
        ));
        if let Some(datasource) = &table.metadata.datasource {
            output.push_str(&format!(
                "- Datasource: `{}`\n",
                escape_markdown(datasource)
            ));
        }
        if let Some(cache_strategy) = &table.metadata.cache_strategy {
            output.push_str(&format!("- Cache: `{}`\n", escape_markdown(cache_strategy)));
        }
        if table.metadata.is_readonly {
            output.push_str("- Readonly: yes\n");
        }
        if let Some(soft_delete_field) = &table.metadata.soft_delete_field {
            output.push_str(&format!(
                "- Soft delete field: `{}`\n",
                escape_markdown(soft_delete_field)
            ));
        }
        output.push('\n');

        output.push_str("| Field | Type | Flags | Constraints |\n");
        output.push_str("|-------|------|-------|-------------|\n");
        for field in &table.fields {
            let mut type_display = field.type_name.clone();
            if field.is_list {
                type_display.push_str("[]");
            }
            if field.is_optional {
                type_display.push('?');
            }
            ...
            output.push_str(&format!(
                ...
                escape_markdown(&type_display),
                flags.join(", "),
                escape_markdown(&constraints)
            ));
        }
        output.push('\n');
        ...
        if !table.indexes.is_empty() {
            ...
            for index in &table.indexes {
                ...
                output.push_str(&format!(
                    "- `{}`: {}({})\n",
                    escape_markdown(&index.name),
                    uniqueness,
                    index
                        .fields
                        .iter()
                        .map(|field| format!("`{}`", escape_markdown(field)))
                        .collect::<Vec<_>>()
                        .join(", ")
                ));
            }
        ...
        if !table.references.is_empty() {
            ...
            for reference in &table.references {
                let alias = reference
                    .alias
                    .as_ref()
                    .map(|alias| format!(" as `{}`", escape_markdown(alias)))
                    .unwrap_or_default();
                output.push_str(&format!(
                    "- `{}` -> `{}` via `{}`{}\n",
                    escape_markdown(&table.name),
                    escape_markdown(&reference.table_fqn),
                    escape_markdown(&reference.field_name),
                    alias
                ));
            }
            output.push('\n');
...
mod tests {
    ...
    #[test]
    fn test_to_markdown_includes_summary_and_tables() {
        let viz = SchemaVisualization {
            tables: vec![TableVisualization {
                name: "Player".to_string(),
                fqn: "game.Player".to_string(),
                namespace: "game".to_string(),
                fields: vec![FieldVisualization {
                    name: "id".to_string(),
            ...
            stats: SchemaStats {
                ...
                relation_count: 0,
                namespace_count: 1,
                tables_by_namespace: HashMap::from([("game".to_string(), 1)]),
            },
        };

        let markdown = to_markdown(&viz);
        assert!(markdown.contains("# Schema Reference"));
        assert!(markdown.contains("| Tables | 1 |"));
        assert!(markdown.contains("### `game.Player`"));
        assert!(markdown.contains("| `id` | `u32` | PK | primary_key |"));
    }
}
