...
*   **제약조건 (Constraints):**
    ...
    *   **`foreign_key(TargetTable.target_field) [as <RelationName>]`**: 이 필드가 `TargetTable`의 `target_field`를 참조하는 외래 키임을 선언합니다. `target_field`는 일반적으로 `TargetTable`의 기본 키입니다. 선택적으로 `as <RelationName>`을 추가하여, `TargetTable`에 이 관계를 역으로 참조할 수 있는 필드(예: `user.posts`)를 자동으로 생성하도록 지시할 수 있습니다. 이를 통해 `relation` 키워드 없이 관계를 정의할 수 있습니다.
...
*   **`string`**
    ...
    *   **제약조건:** `max_length(N)`를 사용하여 최대 길이를 제한할 수 있습니다.
    *   **예시:** `name: string max_length(100);`
...
*   **부동소수점 타입 (Floating-Point Types)**
    ...
    *   **예시:** `temperature: f32;`, `coordinate_x: f64;`
