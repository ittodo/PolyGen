...
#### Rhai 템플릿 실시간 프리뷰어 (GUI)
> 목표: 사용자가 직접 새 언어를 추가할 수 있는 개발 환경
- [ ] 템플릿 에디터 탭 추가 (Monaco Editor)
- [ ] 스키마 + 템플릿 → 생성 코드 실시간 렌더링
- [ ] 각 언어별 템플릿 선택 + 파일 트리 표시
- [ ] 에러/경고 실시간 표시
- [ ] 변수/함수 레퍼런스 패널 (CUSTOMIZATION.md 기반)
- [ ] **새 언어 생성 기능**:
  - [ ] "새 언어 만들기" → `templates/<lang>/` 폴더 + `<lang>.toml` 자동 생성
  - [ ] 템플릿 파일 추가/삭제/이름변경
  - [ ] 저장 시 해당 언어로 즉시 코드 생성 가능
...
| 우선순위 | 작업 | 난이도 | 가치 |
|---------|------|--------|------|
| 🔴 높음 | **Rhai 템플릿 프리뷰어** | 중~고 | 커스터마이징 UX |
