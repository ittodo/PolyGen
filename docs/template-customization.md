...
이 문서는 PolyGen의 PolyTemplate 템플릿과 Rhai helper를 커스터마이징하는 방법을 설명합니다.

---

## 개요

PolyGen은 `.ptpl` PolyTemplate 파일을 렌더링하고, `%logic`/`%if`와 helper 함수에는 Rhai를 사용합니다. 사용자는 템플릿과 `rhai_utils/`를 수정하여 생성되는 코드의 스타일, 명명 규칙, 기능을 변경할 수 있습니다.
...
├── <language>/
│   ├── <lang>.toml           # 언어 설정 파일
│   ├── <lang>_file.ptpl      # 메인 템플릿 (엔티티/구조체)
│   ├── <lang>_*_file.ptpl    # 추가 템플릿 (로더, 컨테이너 등)
│   └── rhai_utils/
...
메인 템플릿(`<lang>_file.ptpl`)에서 생성되는 코드의 형식을 변경합니다.
...
templates/csharp/
├── csharp.toml                    # 설정
├── csharp_file.ptpl               # 엔티티 클래스
├── csharp_container_file.ptpl     # 데이터 컨테이너
├── csharp_csv_mappers_file.ptpl   # CSV 파서
├── csharp_csv_columns_file.ptpl   # CSV 컬럼 정의
├── csharp_binary_readers_file.ptpl # 바이너리 읽기
├── csharp_binary_writers_file.ptpl # 바이너리 쓰기
├── csharp_datacontext_file.ptpl   # DataContext
├── csharp_sqlite_accessor_file.ptpl # SQLite 접근자
└── rhai_utils/
...
**4. Validation 커스터마이징** (`csharp_container_file.ptpl`)
```rhai
// 커스텀 검증 메시지
...
**4. 직렬화 라이브러리** (`cpp_loaders_file.ptpl`)
```rhai
// 기본: nlohmann/json
...
templates/rust/
├── rust.toml                   # 설정
├── rust_file.ptpl              # 구조체/Enum 정의
├── rust_loaders_file.ptpl      # CSV/JSON/Binary 로더
├── rust_container_file.ptpl    # 컨테이너 시스템
├── rust_sqlite_accessor_file.ptpl # SQLite 접근자
└── rhai_utils/
    └── type_mapping.rhai       # 타입 매핑
```
...
**4. 에러 처리** (`rust_loaders_file.ptpl`)
```rhai
// thiserror 사용
...
├── typescript.toml             # 설정
├── typescript_file.ptpl        # 인터페이스/Enum
├── typescript_zod_file.ptpl    # Zod 스키마
├── typescript_sqlite_accessor_file.ptpl # SQLite 접근자
└── rhai_utils/
    └── type_mapping.rhai       # 타입 매핑
```
...
**3. Zod 스키마 커스터마이징** (`typescript_zod_file.ptpl`)
```rhai
// 커스텀 에러 메시지
...
templates/go/
├── go.toml                   # 설정
├── go_file.ptpl              # 구조체/Enum
├── go_container_file.ptpl    # 컨테이너
└── rhai_utils/
    └── type_mapping.rhai     # 타입 매핑
```
...
**4. Validation 추가** (`go_container_file.ptpl`)
```rhai
...
| 문서 | 설명 |
|------|------|
| [README.md](README.md) | 문서 인덱스와 개발 전 필수 확인 문서 |
| [source-structure.md](source-structure.md) | 소스 코드 구조 |
| [AGENTS.md](../AGENTS.md) | 에이전트 작업 지침 |
