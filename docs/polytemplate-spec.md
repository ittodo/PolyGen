...
%if field.has_foreign_key || field.is_unique
```
...
| `all_tables` | List\<Struct\> | Flattened list of structs; legacy `__Enum` helper structs are excluded |
...
| `is_index` | Bool | Legacy field-level index flag. Canonical templates should use `struct.indexes`. |
...
Generated code files receive a top-level `Code generated by PolyGen. DO NOT EDIT.` notice in the
codegen write path. Templates may include their own notice when they need target-specific extra
lines such as source paths; codegen skips adding a duplicate notice if one is already present near
the top of the rendered file. Data outputs that cannot carry comments, such as JSON and CSV, are
left unchanged.
